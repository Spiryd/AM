...
fn main() {
    ...
    for path in paths {
        ...
        //println!("{:?}", &mst);
        let weight = weight(&parent, &adj_matrix);
        println!("{:?}", &weight);
        let mst = parent_to_adj_list(&parent, &adj_matrix);

    }
}

fn weight(tree: &Vec<usize>, adj_matrix: &Vec<Vec<u32>>) -> u32 {
    let mut s = 0;
    for i in 1..(tree.len()) {
        s += adj_matrix[i][tree[i]]
    }
    s
}

fn parent_to_adj_list(parent: &Vec<usize>, adj_matrix: &Vec<Vec<u32>>) {
    todo!()
}

fn prim(adj_matrix: &Vec<Vec<u32>>, point_count: usize) -> Vec<usize> {
    ...
    for _ in 0..(point_count - 1) {
        ...
        for v in 0..point_count {
            if adj_matrix[u][v] != 0 && mst_set[v] == false && adj_matrix[u][v] < key[v] {
                parent[v] = u;
...
fn min_key(key: &Vec<u32>, mst_set: &Vec<bool>, point_count: usize) -> usize {
    let mut min = u32::MAX;
    let mut min_index = 0;
    for v in 0..point_count {
